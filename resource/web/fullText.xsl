...
<xsl:if test="$mode=''">
	<html>
		<head>
            ...
            <link rel="stylesheet" href="http://pubannotation.org/textae/lib/css/textae-4.0.5.min.css" />
            <script src="http://pubannotation.org/textae/vender/jquery.jsPlumb-1.5.2-min.js"></script>
            <script src="http://pubannotation.org/textae/lib/textae-4.0.5.min.js"></script>
			...
			<script type="text/javascript">
                ...
                /* TOP FRAME DOCUMENT MODE */
                    ...
                    function visualizeSent(sent, id) {
                        ...
                        var filename = currentXMLFile.substring(0, currentXMLFile.lastIndexOf('.'));
                        var data = $.getJSON("result.json").responseText;
                        var myJSON = JSON.parse(data);
    


                        var pos = sent - myJSON.offset - 1;
                        var pos2 = 0; 
                        for (var i=0; i<myJSON.sentence[pos].frame.length; i++) {
                            if (myJSON.sentence[pos].frame[i].id-1 == id) {
                                pos2 = i;
                            }
                        }
                        top.frames[1].showSent(myJSON.sentence[pos].frame[pos2]);
                    }
				}
...
/*                    // create paragraph view div
                    var viewDiv = document.createElement("div");
                    viewDiv.setAttribute("class", "textae-editor"); 
                    viewDiv.innerHTML = "";
                    document.body.appendChild(viewDiv);

                    console.log(document);
                    console.log(document.getElementById('viewdiv'));
                    console.log(document.getElementById('viewdiv').innerHTML);
*/ 

                    function showSent(frame_data, frame_name) { 

                        document.body.innerHTML = '';

                        var addon; 
                        addon = document.createElement("link");
                        addon.setAttribute("rel", "stylesheet");
                        addon.setAttribute("href", "http://ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/themes/smoothness/jquery-ui.css");
                        document.body.appendChild(addon);

                        addon = document.createElement("link");
                        addon.setAttribute("rel", "stylesheet");
                        addon.setAttribute("href", "http://pubannotation.org/textae/lib/css/textae-4.0.5.min.css");
                        document.body.appendChild(addon);

                        addon = document.createElement("script"); 
                        addon.setAttribute("src", "http://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js");
                        document.body.appendChild(addon);

                        addon = document.createElement("script"); 
                        addon.setAttribute("src", "http://ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/jquery-ui.min.js");
                        document.body.appendChild(addon);

                        addon = document.createElement("script"); 
                        addon.setAttribute("src", "http://cdnjs.cloudflare.com/ajax/libs/underscore.js/1.6.0/underscore-min.js");
                        document.body.appendChild(addon);

                        addon = document.createElement("script"); 
                        addon.setAttribute("src", "http://pubannotation.org/textae/vender/jquery.jsPlumb-1.5.2-min.js");
                        document.body.appendChild(addon);

                        addon = document.createElement("script"); 
                        addon.setAttribute("src", "http://pubannotation.org/textae/lib/textae-4.0.5.min.js");
                        document.body.appendChild(addon);


                        document.body.innerHTML += '<h2>' + frame_data.kor.frameName + '</h2>'


                        var viewDiv = document.createElement("div");
                        viewDiv.setAttribute("class", "textae-editor"); 
                        viewDiv.setAttribute("id","viewdiv");
                        viewDiv.setAttribute("style", "padding-top:50px");
                        viewDiv.innerHTML = JSON.stringify(frame_data.kor);
                        document.body.appendChild(viewDiv);

                        var viewDiv2 = document.createElement("div");
                        viewDiv2.setAttribute("class", "textae-editor"); 
                        viewDiv2.setAttribute("id","viewdiv2");
                        viewDiv2.setAttribute("style", "padding-top:50px");
                        viewDiv2.innerHTML = JSON.stringify(frame_data.eng);
...
<xsl:if test="$mode='document'">
    ...
    <xsl:for-each select="fn:sentence">
        <!-- have to escape all quotes in the sent first, using template at bottom TODO-->
        ...
        <xsl:for-each select="fn:annotationSet/fn:layer[@name='NER'or@name='WSL'or@name='Target']/fn:label">
            ...
            <xsl:if test="../@name='Target' and @name='Target' and ../../@frameName">
                ...
                <xsl:if test="../../@status = 'MANUAL'">
                    //<![CDATA[
                    insertMapValue(charLabelMap, start, "<TARGET><a href='javascript:visualizeSent(" + sentid + "," + id + ")'>", true);
